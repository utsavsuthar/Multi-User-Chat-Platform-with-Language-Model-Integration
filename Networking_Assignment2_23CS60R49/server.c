...
// void handle_message(int socket_id,char *client_unique_id, char *buffer){
...
void handle_message(int client_index, const char *message)
{
    ...
    char filename[50];
    sprintf(filename, "%s.txt", clients[client_index].client_unique_id);
    ...
    sprintf(command, "python3 gpt_2_gen.py \"%s\"", clients[client_index].client_unique_id);
    // printf("\n%s\n", command);
    ...
    else
    {
        ...
        // fgets(response,BUFFER_SIZE,file);
        ...
        char formatted_response[BUFFER_SIZE + 12];
        snprintf(formatted_response, sizeof(formatted_response), "gpt2bot> %s\n", response);
        send(clients[client_index].socket_id, formatted_response, strlen(formatted_response), 0);
    }
}
            ...
            if (clients[i].status && FD_ISSET(clients[i].socket_id, &readfds))
            {
                ...
                else
                {
                    ...
                    else if (strncmp(buffer, "/chatbot login", 14) == 0)
                    {
                        ...
                        // clients[i].chatbotv2_status = false;
                    ...
                    else if (strncmp(buffer, "/chatbot_v2 login", 18) == 0)
                    {
                        char response[BUFFER_SIZE];
                        memset(response, 0, sizeof(response));
                        clients[i].chatbotv2_status = true;
                        // clients[i].chatbot_status = false;
                        strcat(response, "gpt2bot> Hi, I am updated bot,I am able to answer any question be it corrector incorrect\n");
                        send(client_socket, response, strlen(response), 0);
                    }
                    else if (strncmp(buffer, "/chatbot_v2 logout", 19) == 0)
                    {
                        char response[BUFFER_SIZE];
                        memset(response, 0, sizeof(response));
                        clients[i].chatbotv2_status = false;
                        strcat(response, "gpt2bot> Bye! Have a nice day and hope you do not have any complaints about me\n");
                        send(client_socket, response, strlen(response), 0);
                    }
                    else if (clients[i].chatbotv2_status == true)
                    {
                        // printf("%s", buffer);
                        // char *question;
                    ...
                    else if (strncmp(buffer, "/history_delete", 15) == 0)
                    {
                        ...
                        int flag = 0;
                        if (sscanf(buffer, "%17s %36s", command, dest_id) == 2)
                        {
                    ...
                    else if (strncmp(buffer, "/history", 8) == 0)
                    {
                        ...
                        int flag = 0;
                        if (sscanf(buffer, "%10s %36s", command, dest_id) == 2)
                        {
                            // printf("%s %s\n",command,dest_id);
                            FILE *fptr = fopen(FILENAME, "r");
                            char sender[50], receiver[50], message[200];
                            ...
                            while (fscanf(fptr, "%s\t%s\t%[^\n]", sender, receiver, message) == 3)
                            {
                                ...
                                {
                                    flag = 1;
                                    size_t current_length = strlen(answer);
                                    snprintf(answer + current_length, BUFFER_SIZE, "Sender: %s\tReceiver: %s\tMessage: %s\n", sender, receiver, message);
